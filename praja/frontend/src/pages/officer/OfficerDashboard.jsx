    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ background: 'var(--navy)', padding: '12px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div>
            <div style={{ color: 'white', fontWeight: 800, fontSize: '0.95rem' }}>PRAJA â€” Officer Portal</div>
            <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.62rem' }}>ðŸ‘® {user?.name || user?.full_name}</div>
          </div>
        </div>
      ...
      <div style={{ padding: '20px 24px', maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'var(--card)', borderRadius: '14px', border: '1px solid var(--border)', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
          ...
          {loading ? (
            <div style={{ padding: '40px', textAlign: 'center', color: 'var(--muted)' }}>â³ Loading tickets...</div>
          ) : tickets.length === 0 ? (
                  ...
                  {tickets.map(t => {
                    ...
                    return (
                      <tr key={t.id} style={{ borderBottom: '1px solid #F0F4FA' }}>
                        ...
                        <td style={{ padding: '12px 14px', color: 'var(--muted)', fontSize: '0.8rem' }}>{t.ai_category || t.category || 'â€”'}</td>
                        ...
                        <td style={{ padding: '12px 14px', whiteSpace: 'nowrap' }}>
                          <button onClick={() => updateStatus(t.id, 'in_progress')} style={{ padding: '5px 10px', marginRight: '4px', borderRadius: '6px', border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--text)', cursor: 'pointer', fontSize: '0.75rem' }}>â–¶ Start</button>
                          <button onClick={() => updateStatus(t.id, 'resolved')} style={{ padding: '5px 10px', marginRight: '4px', borderRadius: '6px', border: '1px solid #A3D9B5', background: '#E8F5EE', color: '#1A7340', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 600 }}>âœ… Resolve</button>
                          <button onClick={() => escalate(t.id)} style={{ padding: '5px 10px', borderRadius: '6px', border: '1px solid #F5C6CB', background: '#FDF3F3', color: '#C0392B', cursor: 'pointer', fontSize: '0.75rem' }}>ðŸš¨ Escalate</button>
                        </td>
                      </tr>
                    )
                  })}
