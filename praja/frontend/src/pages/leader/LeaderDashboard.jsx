...
export default function LeaderDashboard() {
  ...
  async function generateBriefing() {
    ...
    try {
      const { data } = await api.get('/officers/briefing')
  ...
  useEffect(() => {
    ...
    if (activeModule === "briefing" && !briefing) generateBriefing()
  }, [activeModule])
      ...
      <div style={{ flex:1, overflow:"auto" }}>
        ...
        <div style={{ padding:24 }}>
          ...
          {activeModule==="briefing"&&(
            <div style={{ background:CARD, border:1px solid , borderRadius:14, padding:30, maxWidth:800, margin:"0 auto" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
                <div>
                  <div style={{ fontWeight:800, fontSize:"1.4rem", color:SAFFRON }}>Daily AI Briefing</div>
                  <div style={{ fontSize:"0.8rem", color:MUTED }}>Generated by LLaMA-3.3-70B</div>
                </div>
                <button 
                  onClick={generateBriefing} 
                  disabled={briefingLoading}
                  style={{ background:SAFFRON, color:"#fff", border:"none", padding:"8px 16px", borderRadius:8, fontWeight:700, cursor:briefingLoading?"not-allowed":"pointer" }}>
                  {briefingLoading ? "Generating..." : "Regenerate"}
                </button>
              </div>
              
              {briefingLoading ? (
                <div style={{ padding:60, textAlign:"center", color:LIGHT }}>
                  <div style={{ fontSize:"2rem", marginBottom:10 }}>??</div>
                  <div>Analyzing grievance data...</div>
                </div>
              ) : (
                <div style={{ fontSize:"0.95rem", lineHeight:1.7, color:TEXT, whiteSpace:"pre-wrap" }}>
                  {briefing}
                </div>
              )}
            </div>
