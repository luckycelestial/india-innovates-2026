"""
WhatsApp Bot via Twilio Sandbox â€” Core webhook and message routing.
...
# â”€â”€ Main webhook â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def whatsapp_webhook(
    ...
    try:
        ...
        if NumMedia > 0 and MediaUrl0:
            if "audio" in MediaContentType0 or "video" in MediaContentType0:
                ...
                if transcription_result.get("text"):
                    ...
                    resp.message("âš ï¸ Apologies, transcription failed. Please try again.")
                    return xml_response(resp)

        if not text_body:
            resp.message("âš ï¸ It seems you sent an empty message I couldn't process. Please send a text or voice note.")
            return xml_response(resp)
        ...
        resp.message(
            f"âš ï¸ PRAJA encountered an error parsing your message.\n"
...
async def whatsapp_process_step_2(
    ...
    try:
        ...
        try:
            ...
            if settings.TWILIO_ACCOUNT_SID:
                TwilioClient(settings.TWILIO_ACCOUNT_SID, settings.TWILIO_AUTH_TOKEN).messages.create(
                    ...
                    body=f"âš ï¸ [DEBUG] Internal error inside handle_message: {handler_err}"
                )
            raise handler_err
        ...
        resp.message(
            f"âš ï¸ PRAJA encountered an error computing your response.\n"
...
# â”€â”€ Background processing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    try:
        ...
        if not transcription_result.get("text"):
            twilio_client.messages.create(
                to=From, from_=settings.TWILIO_WHATSAPP_NUMBER,
                body="âš ï¸ Apologies, transcription failed. Please try again."
            )
            return
...
# â”€â”€ Core message handler â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def _handle_message(Body: str, From: str, resp: MessagingResponse, user_lang: str = None, is_voice: bool = False) -> None:
    ...
    if text.upper() == "CALL ME":
        ...
        try:
            ...
            resp.message(translate_from_english("ðŸ“ž *Initiating Voice Call...*\n\nPlease answer your phone to file your complaint via our IVR system.", detected_lang))
        except Exception as e:
            resp.message(f"âš ï¸ *Call Failed:* {str(e)}")
        return
        ...
        if created_at:
            try:
                ...
                if g.get("status") == "resolved":
                    ...
                    sla_hours_left = "\n\U0001f6a8 *SLA:* BREACHED â€” escalation triggered"
            except Exception:
                pass
        reply = (
            f"{s_emoji} *Complaint Status â€” PRAJA*\n\n"
    ...
    if text.lower() == "status":
        ...
        if not rows.data:
            ...
            msg = "ðŸ“Š *Your Recent Complaints*\n\n"
            for g in rows.data:
                msg += f"ðŸ”¹ *{g['tracking_id']}* - {g['status'].title()}\n   {g.get('title', 'No title')[:30]}...\n\n"
            resp.message(msg)
        return
    ...
    if drafts.data:
        ...
        ai_resp = agentic_chat_with_groq(history, user_name)
...
def _reply_ai_confirmation(resp, reply_fn, classification, detected_lang, location_text, tracking_id):
    ...
    sentiment_emoji = {"Negative": "ðŸ˜¡", "Positive": "ðŸ˜„", "Neutral": "ðŸ˜"}.get(sentiment, "ðŸ’¬")

    reply = (
        f"ðŸ¤– *AI Processing Complete*\n"
        f"â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€\n\n"
        f"âœ… Language: *{detected_lang}*\n"
        f"ðŸ·ï¸ Department: *{classification.get('category', 'General')}*\n"
        f"âš¡ Priority: *{prio.title()} ({prio_label})*\n"
        f"ðŸ“ Location: *{location_text}*\n"
        f"{sentiment_emoji} Sentiment: *{sentiment}*\n\n"
        f"â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€\n"
        f"ðŸ†” Ticket *{tracking_id}* created.\n"
        f"ðŸ“¤ Routed to *{classification.get('category', 'General')} Department*.\n"
        f"ðŸ“² Officer notified.\n\n"
        f"ðŸŽ¯ *SLA Timeline:* 72 hours to resolve\n\n"
