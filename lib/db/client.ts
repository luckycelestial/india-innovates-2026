class LocalDbBuilder {
  ...
  async then(onfulfilled?: (value: any) => any) {
    try {
      ...
      if (this.tableName === 'complaints' && data) {
        const normalizeStatus = (status: string): string => {
          ...
          if (s === 'resolved' || s === 'pending_review') return 'resolved'
...
class LocalDbClient {
  auth = {
    async getUser() {
      ...
      if (typeof document !== 'undefined') {
        ...
        if (roleCookie) {
          cookieRole = roleCookie.split('=')[1].trim();
        }
      }

      const path = window.location.pathname;

      if (cookieRole === 'citizen') {
        return {
          data: {
            user: {
              id: 'citizen-id-123',
          ...
          return {
            data: {
              user: {
                ...
                user_metadata: { name: 'Priya Nair', role: 'admin' }
              }
            },
            error: null
          };
  ...
  from(tableName: string) {
    return new LocalDbBuilder(tableName);
  }
